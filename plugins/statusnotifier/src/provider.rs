...
use crate::STATUS_NOTIFIER_ITEM_KIND;
use crate::state::{SharedStatusNotifierState, StatusNotifierState};

#[derive(Clone)]
pub struct StatusNotifierProvider {
    kind: NodeKind,
    state: SharedStatusNotifierState,
}

impl StatusNotifierProvider {
    pub(crate) fn new(kind: NodeKind, state: SharedStatusNotifierState) -> Self {
        Self { kind, state }
    }

    async fn with_state<T>(
        &self,
        operation: impl FnOnce(&StatusNotifierState) -> Result<T>,
    ) -> Result<T> {
...
#[async_trait]
impl NodeProvider for StatusNotifierProvider {
    fn kind(&self) -> &NodeKind {
        &self.kind
    }

    fn access(&self) -> NodeAccess {
        if self.kind.as_str() == STATUS_NOTIFIER_ITEM_KIND {
            NodeAccess::hidden()
        } else {
...
#[async_trait]
impl PathProvider for StatusNotifierProvider {
    fn kind(&self) -> &NodeKind {
        &self.kind
    }
...
#[async_trait]
impl PropertyProvider for StatusNotifierProvider {
...
#[async_trait]
impl RelationProvider for StatusNotifierProvider {

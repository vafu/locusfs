...
pub async fn register(graph: &DynamicGraph) -> Result<MprisPluginHandle> {
    ...
    register_with_runtime(graph, runtime).await
}

async fn register_with_runtime(
    graph: &DynamicGraph,
    runtime: tokio::runtime::Handle,
) -> Result<MprisPluginHandle> {
    let (state, event_stream) = MprisRuntime::start(graph.clone(), runtime);
...
impl LocusFsPlugin for MprisPlugin {
    ...
    async fn register(
        ...
        Ok(Box::new(
            register_with_runtime(&context.graph, context.runtime).await?,
        ))
    }
}
